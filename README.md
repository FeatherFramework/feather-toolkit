...
Contract 1 includes models, objects, peds, blips, key listeners, prompts, 2D/3D text, and clipboard access. Teleport, horses, and wagons remain domain-owned. All stateful handles belong to the resource that created them; cross-resource mutation is rejected and owned handles are released when that resource stops.
